...
class CPTO { 
	...
	public function posts_orderby( $order_by, $query ) { 
		...
		if ( ( is_admin() && ! wp_doing_ajax() ) || ( wp_doing_ajax() && isset( $_REQUEST['action'] ) && 'query-attachments' == $_REQUEST['action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			...
			if ( '1' == $options['adminsort'] || ( wp_doing_ajax() && isset( $_REQUEST['action'] ) && 'query-attachments' == $_REQUEST['action'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				...
				if ( is_object( $post ) && 'acf-field-group' == $post->post_type || ( defined( 'DOING_AJAX' ) && isset( $_REQUEST['action'] ) && 0 === strpos( sanitize_text_field( $_REQUEST['action'] ), 'acf/' ) ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
							return $order_by;
				}
	...
	public function admin_configure_notices() {
		...
		?>
						<div class="error fade">
							<p><strong><?php esc_html_e( 'Post Types Order must be configured. Please go to', 'post-types-order' ); ?> <a href="<?php echo esc_url( get_admin_url() ); ?>options-general.php?page=cpto-options"><?php esc_html_e( 'Settings Page', 'post-types-order' ); ?></a> <?php esc_html_e( 'make the configuration and save', 'post-types-order' ); ?></strong></p>
						</div>
				<?php
	}
	...
	public function plugin_options_menu() {
			...
			$hook_id = add_options_page( 'Post Types Order', '<img class="menu_pto" src="' . CPTURL . '/images/menu-icon.png" alt="" /> Post Types Order', 'manage_options', 'cpto-options', array( $options_interface, 'plugin_options_interface' ) );
	...
	public function admin_init() { 
		if ( isset( $_GET['page'] ) && 'order-post-types-' == substr( sanitize_text_field( $_GET['page'] ), 0, 17 ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
				$this->current_post_type = get_post_type_object( str_replace( 'order-post-types-', '', sanitize_text_field( $_GET['page'] ) ) ); // phpcs:ignore WordPress.Security.NonceVerification.Recommended
	...
	public function save_ajax_order() { 
			...
			$nonce = isset( $_POST['interface_sort_nonce'] ) ? sanitize_text_field( $_POST['interface_sort_nonce'] ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified.
		...
		if ( ! wp_verify_nonce( $nonce, 'interface_sort_nonce' ) ) {
			...
			parse_str( ( isset( $_POST['order'] ) ? sanitize_text_field( $_POST['order'] ) : '' ), $data );
	...
	public function save_archive_ajax_order() {
			...
			$post_type = isset( $_POST['post_type'] ) ? preg_replace( '/[^a-zA-Z0-9_\-]/', '', sanitize_text_field( $_POST['post_type'] ) ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified.
			$paged     = isset( $_POST['paged'] ) ? filter_var( sanitize_text_field( $_POST['paged'] ), FILTER_SANITIZE_NUMBER_INT ) : ''; // phpcs:ignore WordPress.Security.NonceVerification.Missing -- Nonce is verified.
			$nonce     = ( isset( $_POST['archive_sort_nonce'] ) ) ? sanitize_text_field( $_POST['archive_sort_nonce'] ) : ''; 
		...
		if ( ! wp_verify_nonce( $nonce, 'CPTO_archive_sort_nonce_' . $userdata->ID ) ) {
			...
			parse_str( sanitize_text_field( isset( $_POST['order'] ) ?? sanitize_text_field( $_POST['order'] ) ), $data );
